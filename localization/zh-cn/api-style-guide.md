...
本文档中的关键字 "必须（MUST）"、"不得（MUST NOT）"、"需要（REQUIRED）"、"应当（SHALL）"、"不应（SHALL NOT）"、"应该（SHOULD）"、"不该（SHOULD NOT）", "建议的（RECOMMANDED）", "可以（MAY）", and "可选的（OPTIANAL）" 的含义在标准 [RFC 2119](https://www.ietf.org/rfc/rfc2119.txt) 中被解释和描述。
